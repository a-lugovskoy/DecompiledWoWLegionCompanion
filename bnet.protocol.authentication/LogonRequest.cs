...
namespace bnet.protocol.authentication
{
	public class LogonRequest : IProtoBuf
	{
		...
		public uint GetSerializedSize()
		{
			...
			{
				num += 1u;
				uint byteCount2 = (uint)Encoding.get_UTF8().GetByteCount(this.Platform);
				num += ProtocolParser.SizeOfUInt32(byteCount2) + byteCount2;
			}
			if (this.HasLocale)
			{
				num += 1u;
				uint byteCount3 = (uint)Encoding.get_UTF8().GetByteCount(this.Locale);
				num += ProtocolParser.SizeOfUInt32(byteCount3) + byteCount3;
			}
			if (this.HasEmail)
			{
				num += 1u;
				uint byteCount4 = (uint)Encoding.get_UTF8().GetByteCount(this.Email);
				num += ProtocolParser.SizeOfUInt32(byteCount4) + byteCount4;
			}
			if (this.HasVersion)
			{
				num += 1u;
				uint byteCount5 = (uint)Encoding.get_UTF8().GetByteCount(this.Version);
			...
			{
				num += 1u;
				uint byteCount6 = (uint)Encoding.get_UTF8().GetByteCount(this.UserAgent);
