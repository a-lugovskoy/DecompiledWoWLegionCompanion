...
namespace bnet.protocol.account
{
	public class Wallet : IProtoBuf
	{
		...
		public static void Serialize(Stream stream, Wallet instance)
		{
			...
			{
				stream.WriteByte(34);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.Description));
			}
			stream.WriteByte(40);
			...
			{
				stream.WriteByte(50);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.State));
			}
			if (instance.HasCity)
			{
				stream.WriteByte(58);
			...
			{
				stream.WriteByte(66);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.PostalCode));
			}
			...
			{
				stream.WriteByte(82);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.Bin));
			}
			if (instance.HasLocaleId)
			{
				stream.WriteByte(90);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.LocaleId));
			}
			if (instance.HasStreet)
			{
				stream.WriteByte(98);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.Street));
			}
			if (instance.HasFirstName)
			{
				stream.WriteByte(106);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.FirstName));
			}
			if (instance.HasLastName)
			{
				stream.WriteByte(114);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.LastName));
			}
		...
		public uint GetSerializedSize()
		{
			...
			{
				num += 1u;
				uint byteCount = (uint)Encoding.get_UTF8().GetByteCount(this.Description);
			...
			{
				num += 1u;
				uint byteCount2 = (uint)Encoding.get_UTF8().GetByteCount(this.State);
				num += ProtocolParser.SizeOfUInt32(byteCount2) + byteCount2;
			}
			if (this.HasCity)
			{
				num += 1u;
				uint byteCount3 = (uint)Encoding.get_UTF8().GetByteCount(this.City);
				num += ProtocolParser.SizeOfUInt32(byteCount3) + byteCount3;
			}
			if (this.HasPostalCode)
			{
				num += 1u;
				uint byteCount4 = (uint)Encoding.get_UTF8().GetByteCount(this.PostalCode);
			...
			{
				num += 1u;
				uint byteCount5 = (uint)Encoding.get_UTF8().GetByteCount(this.Bin);
				num += ProtocolParser.SizeOfUInt32(byteCount5) + byteCount5;
			}
			if (this.HasLocaleId)
			{
				num += 1u;
				uint byteCount6 = (uint)Encoding.get_UTF8().GetByteCount(this.LocaleId);
				num += ProtocolParser.SizeOfUInt32(byteCount6) + byteCount6;
			}
			if (this.HasStreet)
			{
				num += 1u;
				uint byteCount7 = (uint)Encoding.get_UTF8().GetByteCount(this.Street);
				num += ProtocolParser.SizeOfUInt32(byteCount7) + byteCount7;
			}
			if (this.HasFirstName)
			{
				num += 1u;
				uint byteCount8 = (uint)Encoding.get_UTF8().GetByteCount(this.FirstName);
				num += ProtocolParser.SizeOfUInt32(byteCount8) + byteCount8;
			}
			if (this.HasLastName)
			{
				num += 1u;
				uint byteCount9 = (uint)Encoding.get_UTF8().GetByteCount(this.LastName);
