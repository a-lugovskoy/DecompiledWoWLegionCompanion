...
namespace Newtonsoft.Json.Bson
{
	public class BsonReader : JsonReader
	{
		...
		public override byte[] ReadAsBytes()
		{
			...
			throw new JsonReaderException("Error reading bytes. Expected bytes but got {0}.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
			{
				this.TokenType
			}));
		}
		...
		private bool IsWrappedInTypeObject()
		{
			if (this.TokenType == JsonToken.StartObject)
			{
				...
				throw new JsonReaderException("Unexpected token when reading bytes: {0}.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
				{
		...
		public override decimal? ReadAsDecimal()
		{
			...
			{
				this.SetToken(JsonToken.Float, Convert.ToDecimal(this.Value, CultureInfo.get_InvariantCulture()));
				return new decimal?((decimal)this.Value);
			}
			throw new JsonReaderException("Error reading decimal. Expected a number but got {0}.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
			{
				this.TokenType
			}));
		}
		...
		public override DateTimeOffset? ReadAsDateTimeOffset()
		{
			...
			throw new JsonReaderException("Error reading date. Expected bytes but got {0}.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
			{
				this.TokenType
			}));
		}
		...
		public override bool Read()
		{
			...
			try
			{
				...
				case BsonReader.BsonReaderState.CodeWScopeScopeEnd:
					...
					break;
				default:
					throw new JsonReaderException("Unexpected state: {0}".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
					{
		...
		private string ReadString()
		{
			...
			while (true)
			{
				...
				int chars = Encoding.get_UTF8().GetChars(this._byteBuffer, 0, lastFullCharStop + 1, this._charBuffer, 0);
			...
			int chars2 = Encoding.get_UTF8().GetChars(this._byteBuffer, 0, num4, this._charBuffer, 0);
		...
		private string GetString(int length)
		{
			...
			while (true)
			{
				int num3 = (length - num <= 128 - num2) ? (length - num) : (128 - num2);
				num4 = this._reader.get_BaseStream().Read(this._byteBuffer, num2, num3);
				if (num4 == 0)
				{
					break;
				}
		...
		private void EnsureBuffers()
		{
			...
			{
				int maxCharCount = Encoding.get_UTF8().GetMaxCharCount(128);
