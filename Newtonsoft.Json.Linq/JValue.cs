...
namespace Newtonsoft.Json.Linq
{
	public class JValue : JToken, IFormattable, IComparable, IEquatable<JValue>, IComparable<JValue>
	{
		...
		private static int Compare(JTokenType valueType, object objA, object objB)
		{
			...
			case JTokenType.Raw:
			{
				string text = Convert.ToString(objA, CultureInfo.get_InvariantCulture());
				string text2 = Convert.ToString(objB, CultureInfo.get_InvariantCulture());
				return text.CompareTo(text2);
			}
			case JTokenType.Integer:
				if (objA is ulong || objB is ulong || objA is decimal || objB is decimal)
				{
					return Convert.ToDecimal(objA, CultureInfo.get_InvariantCulture()).CompareTo(Convert.ToDecimal(objB, CultureInfo.get_InvariantCulture()));
				}
				if (objA is float || objB is float || objA is double || objB is double)
				{
					return JValue.CompareFloat(objA, objB);
				}
				return Convert.ToInt64(objA, CultureInfo.get_InvariantCulture()).CompareTo(Convert.ToInt64(objB, CultureInfo.get_InvariantCulture()));
			case JTokenType.Float:
				return JValue.CompareFloat(objA, objB);
			case JTokenType.Boolean:
			{
				bool flag = Convert.ToBoolean(objA, CultureInfo.get_InvariantCulture());
				bool flag2 = Convert.ToBoolean(objB, CultureInfo.get_InvariantCulture());
			...
			case JTokenType.Date:
			{
				if (objA is DateTime)
				{
					DateTime dateTime = Convert.ToDateTime(objA, CultureInfo.get_InvariantCulture());
					DateTime dateTime2 = Convert.ToDateTime(objB, CultureInfo.get_InvariantCulture());
			...
			case JTokenType.Uri:
			{
				...
				Uri uri = (Uri)objA;
				Uri uri2 = (Uri)objB;
				return Comparer<string>.get_Default().Compare(uri.ToString(), uri2.ToString());
			}
			...
			throw MiscellaneousUtils.CreateArgumentOutOfRangeException("valueType", valueType, "Unexpected value type: {0}".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
			{
				valueType
			}));
		}
		...
		{
			double d = Convert.ToDouble(objA, CultureInfo.get_InvariantCulture());
			double num = Convert.ToDouble(objB, CultureInfo.get_InvariantCulture());
		...
		private static JTokenType GetValueType(JTokenType? current, object value)
		{
			...
			throw new ArgumentException("Could not determine JSON object type for type {0}.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
			{
				value.GetType()
			}));
		}
		...
		private static JTokenType GetStringValueType(JTokenType? current)
		{
			...
			JTokenType value = current.get_Value();
			switch (value)
			{
			case JTokenType.Comment:
			case JTokenType.String:
