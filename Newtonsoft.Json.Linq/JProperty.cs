...
namespace Newtonsoft.Json.Linq
{
	public class JProperty : JContainer
	{
		...
		public new JToken Value
		{
			...
			get
			{
				return (this.ChildrenTokens.get_Count() <= 0) ? null : this.ChildrenTokens.get_Item(0);
			}
			set
			{
				base.CheckReentrancy();
				JToken item = value ?? new JValue(null);
				if (this.ChildrenTokens.get_Count() == 0)
				{
		...
		{
			throw new Exception("Cannot add or remove items from {0}.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
			{
				typeof(JProperty)
			}));
		}
		...
		internal override void InsertItem(int index, JToken item)
		{
			if (this.Value != null)
			{
				throw new Exception("{0} cannot have multiple values.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
				{
					typeof(JProperty)
				}));
			}
		...
		public static JProperty Load(JsonReader reader)
		{
			...
			{
				throw new Exception("Error reading JProperty from JsonReader. Current JsonReader item is not a property: {0}".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
				{
					reader.TokenType
				}));
			}
