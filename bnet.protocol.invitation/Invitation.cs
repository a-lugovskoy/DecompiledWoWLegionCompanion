...
namespace bnet.protocol.invitation
{
	public class Invitation : IProtoBuf
	{
		...
		public static void Serialize(Stream stream, Invitation instance)
		{
			...
			{
				stream.WriteByte(50);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.InvitationMessage));
			}
		...
		public uint GetSerializedSize()
		{
			...
			{
				num += 1u;
				uint byteCount = (uint)Encoding.get_UTF8().GetByteCount(this.InviterName);
				num += ProtocolParser.SizeOfUInt32(byteCount) + byteCount;
			}
			if (this.HasInviteeName)
			{
				num += 1u;
				uint byteCount2 = (uint)Encoding.get_UTF8().GetByteCount(this.InviteeName);
				num += ProtocolParser.SizeOfUInt32(byteCount2) + byteCount2;
			}
			if (this.HasInvitationMessage)
			{
				num += 1u;
				uint byteCount3 = (uint)Encoding.get_UTF8().GetByteCount(this.InvitationMessage);
