...
namespace bnet.protocol.invitation
{
	public class Suggestion : IProtoBuf
	{
		...
		public static void Serialize(Stream stream, Suggestion instance)
		{
			...
			{
				stream.WriteByte(34);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.SuggesterName));
			}
			if (instance.HasSuggesteeName)
			{
				stream.WriteByte(42);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.SuggesteeName));
			}
		...
		public uint GetSerializedSize()
		{
			...
			{
				num += 1u;
				uint byteCount = (uint)Encoding.get_UTF8().GetByteCount(this.SuggesterName);
				num += ProtocolParser.SizeOfUInt32(byteCount) + byteCount;
			}
			if (this.HasSuggesteeName)
			{
				num += 1u;
				uint byteCount2 = (uint)Encoding.get_UTF8().GetByteCount(this.SuggesteeName);
