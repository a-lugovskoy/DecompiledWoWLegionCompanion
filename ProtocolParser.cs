...
public static class ProtocolParser
{
	...
	{
		return Encoding.get_UTF8().GetString(ProtocolParser.ReadBytes(stream));
	}
	...
	{
		ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(val));
	}
	...
	public static byte[] ReadValueBytes(Stream stream, Key key)
	{
		...
		case Wire.LengthDelimited:
		{
			...
			using (MemoryStream memoryStream = new MemoryStream())
			{
				...
				array = new byte[(ulong)num + (ulong)memoryStream.get_Length()];
				memoryStream.ToArray().CopyTo(array, 0);
				i = (int)memoryStream.get_Length();
			}
