...
namespace bnet.protocol.attribute
{
	public class Variant : IProtoBuf
	{
		...
		public static void Serialize(Stream stream, Variant instance)
		{
			...
			{
				stream.WriteByte(42);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.StringValue));
			}
			...
			{
				stream.WriteByte(66);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.FourccValue));
			}
		...
		public uint GetSerializedSize()
		{
			...
			{
				num += 1u;
				uint byteCount = (uint)Encoding.get_UTF8().GetByteCount(this.StringValue);
			...
			{
				num += 1u;
				uint byteCount2 = (uint)Encoding.get_UTF8().GetByteCount(this.FourccValue);
