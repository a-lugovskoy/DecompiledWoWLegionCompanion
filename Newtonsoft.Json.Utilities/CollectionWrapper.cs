	...
	internal class CollectionWrapper<T> : IEnumerable, IWrappedCollection, IList, ICollection, ICollection<T>, IEnumerable<T>
	{
		...
		bool IList.IsFixedSize
		{
			get
			{
				...
				{
					return this._genericCollection.get_IsReadOnly();
				}
				return this._list.get_IsFixedSize();
			}
		}
		...
		object IList.Item
		{
			...
			set
			{
				...
				this._list.set_Item(index, (T)((object)value));
			}
		}
		...
		public virtual int Count
		{
			get
			{
				if (this._genericCollection != null)
				{
					return this._genericCollection.get_Count();
				}
		...
		{
			return value is T || (value == null && (!typeof(T).get_IsValueType() || ReflectionUtils.IsNullableType(typeof(T))));
		}
	}
}
