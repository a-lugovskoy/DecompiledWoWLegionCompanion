...
namespace bnet.protocol.challenge
{
	public class Challenge : IProtoBuf
	{
		...
		public static void Serialize(Stream stream, Challenge instance)
		{
			...
			{
				stream.WriteByte(18);
				ProtocolParser.WriteBytes(stream, Encoding.get_UTF8().GetBytes(instance.Info));
			}
		...
		public uint GetSerializedSize()
		{
			...
			{
				num += 1u;
				uint byteCount = (uint)Encoding.get_UTF8().GetByteCount(this.Info);
				num += ProtocolParser.SizeOfUInt32(byteCount) + byteCount;
			}
			if (this.HasAnswer)
			{
				num += 1u;
				uint byteCount2 = (uint)Encoding.get_UTF8().GetByteCount(this.Answer);
