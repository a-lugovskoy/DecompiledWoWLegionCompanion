...
namespace bgs
{
	public class RPCConnectionMetering
	{
		...
		{
			private uint m_serviceId = 4294967295u;

			private uint m_methodId = 4294967295u;
		...
		{
			private uint m_finiteCallsLeft = 4294967295u;
			...
			{
				string text = (this.StaticData == null || this.StaticData.ServiceName == null) ? "<null>" : this.StaticData.ServiceName;
				string text2 = (this.StaticData == null || this.StaticData.MethodName == null) ? "<null>" : this.StaticData.MethodName;
		...
		private class MeteringData
		{
			...
			public RPCConnectionMetering.RuntimeData GetRuntimeData(uint id)
			{
				...
				if (this.m_runtimeData.TryGetValue(id, ref result))
				{
					return result;
				}
				return null;
		...
		public void SetConnectionMeteringData(byte[] data, ServiceCollectionHelper serviceHelper)
		{
			...
			try
			{
				RPCMeterConfig rPCMeterConfig = RPCMeterConfigParser.ParseConfig(Encoding.get_ASCII().GetString(data));
		...
		private bool UpdateConfigStats(RPCMeterConfig config)
		{
			...
			for (int i = 0; i < methodCount; i++)
			{
				RPCMethodConfig method = config.Method.get_Item(i);
