...
namespace bgs
{
	public class AccountAPI : BattleNetAPI
	{
		...
		private uint m_preferredRegion = 4294967295u;
		...
		public bool HasLicenses
		{
			get
			{
				return this.m_licenses != null && this.m_licenses.get_Count() > 0;
			}
		}
		...
		private void GetAccountStateCallback(RPCContext context)
		{
			...
			if (getAccountStateRequest != null && getAccountStateRequest.EntityId == this.m_battleNet.AccountId)
			{
				...
				if (accountLevelInfo.LicensesList.get_Count() > 0)
				{
					this.m_licenses.Clear();
					base.ApiLog.LogDebug("Found {0} licenses.", new object[]
					{
						accountLevelInfo.LicensesList.get_Count()
					});
					for (int i = 0; i < accountLevelInfo.LicensesList.get_Count(); i++)
					{
						AccountLicense accountLicense = accountLevelInfo.LicensesList.get_Item(i);
