...
namespace bgs
{
	public class BattleNetCSharp : IBattleNet
	{
		public enum ConnectionState
		{
			...
			Logon = 4,
			WaitForLogon = 5,
			WaitForGameAccountSelect = 6,
			WaitForAPIToInitialize = 7,
			Ready = 8,
			Error = 9
		}
		...
		private readonly DateTime m_unixEpoch = new DateTime(1970, 1, 1, 0, 0, 0, 1);
		...
		private uint m_connectedRegion = 4294967295u;
		...
		{
			return (long)(DateTime.get_UtcNow() - this.m_unixEpoch).get_TotalSeconds();
		}

		public double GetRealTimeSinceStartup()
		{
			return this.m_stopwatch.get_Elapsed().get_TotalSeconds();
		}
		...
		public bool Init(bool internalMode, string userEmailAddress, string targetServer, int port, SslParameters sslParams, ClientInterface ci)
		{
			...
			{
				this.m_logSource.LogError("Exception within GetHostAddress: " + ex.get_Message());
			}
			if (flag)
			{
		...
		public void OnConnectHandlerCallback(BattleNetErrors error)
		{
			...
			using (List<BattleNetAPI>.Enumerator enumerator = this.m_apiList.GetEnumerator())
			{
				...
				{
					BattleNetAPI current = enumerator.get_Current();
					current.OnConnected(error);
				}
			}
		}
		...
		private void InitRPCListeners()
		{
			...
			using (List<BattleNetAPI>.Enumerator enumerator = this.m_apiList.GetEnumerator())
			{
				...
				{
					BattleNetAPI current = enumerator.get_Current();
					current.InitRPCListeners(this.m_rpcConnection);
				}
			}
		}
		...
		public void ProcessAurora()
		{
			...
			if (this.m_stateHandlers.TryGetValue(this.m_connectionState, ref auroraStateHandler))
			{
				auroraStateHandler();
			}
			else
			...
			for (int i = 0; i < this.m_apiList.get_Count(); i++)
			{
				BattleNetAPI battleNetAPI = this.m_apiList.get_Item(i);
		...
		{
			return this.m_errorEvents.get_Count();
		}
		...
		{
			return this.m_bnetEvents.get_Count();
		}
		...
		private void HandleNotificationReceived(RPCContext context)
		{
			...
			if (this.m_notificationHandlers.TryGetValue(notification.Type, ref notificationHandler))
			{
		...
		private void OnConnectCallback(RPCContext context)
		{
			...
			{
				int num = 0;
				using (List<uint>.Enumerator enumerator = connectResponse.BindResponse.ImportedServiceIdList.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						uint current = enumerator.get_Current();
						ServiceDescriptor serviceDescriptor = this.m_importedServices.get_Item(num++);
						serviceDescriptor.Id = current;
						this.m_rpcConnection.serviceHelper.AddImportedService(current, serviceDescriptor);
		...
		private BindRequest CreateBindRequest(List<ServiceDescriptor> imports, List<ServiceDescriptor> exports)
		{
			...
			using (List<ServiceDescriptor>.Enumerator enumerator = imports.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					ServiceDescriptor current = enumerator.get_Current();
					bindRequest.AddImportedServiceHash(current.Hash);
				}
			}
			uint num = 0u;
			using (List<ServiceDescriptor>.Enumerator enumerator2 = exports.GetEnumerator())
			{
				while (enumerator2.MoveNext())
				{
					ServiceDescriptor current2 = enumerator2.get_Current();
					num = (current2.Id = num + 1u);
					BoundService boundService = new BoundService();
					boundService.SetId(current2.Id);
					boundService.SetHash(current2.Hash);
					bindRequest.AddExportedService(boundService);
					this.m_rpcConnection.serviceHelper.AddExportedService(current2.Id, current2);
		...
		public void OnBroadcastReceived(IList<Attribute> AttributeList)
		{
		...
		private void PrintBindServiceResponse(BindResponse response)
		{
			...
			{
				text = text + " Id:" + response.ImportedServiceId.get_Item(i);
			}
			text += " ]";
			text += " }";
