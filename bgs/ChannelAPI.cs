...
namespace bgs
{
	public class ChannelAPI : BattleNetAPI
	{
		...
		{
			INVITATION_SERVICE_TYPE_NONE = 0,
			INVITATION_SERVICE_TYPE_PARTY = 1,
			INVITATION_SERVICE_TYPE_CHAT = 2,
			INVITATION_SERVICE_TYPE_GAMES = 3
		}
		...
		{
			return Enumerable.ToArray<ChannelAPI.ReceivedInvite>(Enumerable.SelectMany<List<ChannelAPI.ReceivedInvite>, ChannelAPI.ReceivedInvite>(this.m_receivedInvitations.Values, (List<ChannelAPI.ReceivedInvite> l) => l));
		}
		...
		public void RemoveInviteRequestsFor(EntityId channelId, EntityId suggesteeId, uint removeReason)
		{
			...
			if (this.m_receivedInviteRequests.TryGetValue(channelId, out list))
			{
				...
				{
					Suggestion suggestion = list.get_Item(i);
		...
		private void HandleChannelSubscriber_NotifyRemove(RPCContext context)
		{
			...
			if (channelData != null)
			{
				foreach (Member current in channelData.m_members.Values)
				{
					if (!this.m_battleNet.GameAccountId.Equals(current.Identity.GameAccountId))
					{
						this.m_battleNet.Presence.PresenceUnsubscribe(current.Identity.GameAccountId);
					}
				}
			}
			IL_144:
