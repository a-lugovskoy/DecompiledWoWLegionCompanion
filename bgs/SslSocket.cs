...
namespace bgs
{
	public class SslSocket
	{
		...
		{
			OK = 0,
			FAILED_SERVER_RESPONSE = 1,
			FAILED_CERT_BUNDLE = 2
		}
		...
		public bool Connected
		{
			get
			{
				return this.Socket != null && this.Socket.get_Connected();
			}
		}
		...
		static SslSocket()
		{
			...
			byte[] expr_1A = new byte[4];
			expr_1A[0] = 1;
			expr_1A[2] = 1;
			SslSocket.s_standardPublicExponent = expr_1A;
			...
			byte[] expr_47 = new byte[4];
			expr_47[0] = 1;
			expr_47[2] = 1;
			SslSocket.s_debugPublicExponent = expr_47;
		...
		private void ResolveSSLAddress()
		{
			...
			{
				this.m_address = ((this.m_connection.ResolvedAddress.get_AddressFamily() != 23) ? ("::ffff:" + this.m_connection.ResolvedAddress.ToString()) : this.m_connection.ResolvedAddress.ToString());
			}
			else
			{
		...
		{
			SslSocket.BeginSendDelegate beginSendDelegate = (SslSocket.BeginSendDelegate)ar.get_AsyncState();
		...
		{
			SslSocket.BeginReceiveDelegate beginReceiveDelegate = (SslSocket.BeginReceiveDelegate)ar.get_AsyncState();
			if (this.Socket == null || this.m_sslStream == null)
			{
				if (beginReceiveDelegate != null)
				{
		...
		private static SslSocket.HexStrToBytesError HexStrToBytes(string hex, out byte[] outBytes)
		{
			...
			while (i < length)
			{
				string text = hex.Substring(i, 2);
				outBytes[num] = Convert.ToByte(text, 16);
				i += 2;
				num++;
			}
		...
		private static List<string> GetCommonNamesFromCertSubject(string certSubject)
		{
			...
			string[] array2 = certSubject.Split(array);
		...
		private static bool GetBundleInfo(byte[] unsignedBundleBytes, out SslSocket.BundleInfo info)
		{
			...
			string text = null;
			string @string = Encoding.get_ASCII().GetString(unsignedBundleBytes);
			try
			{
				JsonNode jsonNode = Json.Deserialize(@string) as JsonNode;
				JsonList jsonList = jsonNode["PublicKeys"] as JsonList;
				using (List<object>.Enumerator enumerator = jsonList.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						JsonNode jsonNode2 = (JsonNode)enumerator.get_Current();
						string text2 = (string)jsonNode2["Uri"];
						string hex = (string)jsonNode2["ShaHashPublicKeyInfo"];
						byte[] array = null;
						SslSocket.HexStrToBytesError hexStrToBytesError = SslSocket.HexStrToBytes(hex, out array);
		...
		private static bool IsWhitelistedInCertBundle(SslSocket.BundleInfo bundleInfo, string uri, byte[] publicKey)
		{
			...
			byte[] array = sHA.ComputeHash(publicKey);
			for (int i = 0; i < bundleInfo.bundleKeyHashs.get_Count(); i++)
			{
				byte[] array2 = bundleInfo.bundleKeyHashs.get_Item(i);
				if (Enumerable.SequenceEqual<byte>(array, array2))
				{
					string text = bundleInfo.bundleUris.get_Item(i);
					if (text.Equals(uri))
					{
						return true;
					}
		...
		{
			string issuer = cert.get_Issuer();
			char[] array = new char[]
			{
				','
			};
			string[] array2 = issuer.Split(array);
			...
			return hashSet.get_Count() == 0;
		}

		private static byte[] GetUnsignedBundleBytes(byte[] signedBundleBytes)
		{
			int num = signedBundleBytes.Length - (SslSocket.s_magicBundleSignaturePreamble.get_Length() + 256);
			if (num <= 0)
			{
				return null;
			}
		...
		{
			int num = signedBundleData.Length - (SslSocket.s_magicBundleSignaturePreamble.get_Length() + 256);
			if (num <= 0)
			{
				return false;
			}
			byte[] bytes = Encoding.get_ASCII().GetBytes(SslSocket.s_magicBundleSignaturePreamble);
			...
			string text = "Blizzard Certificate Bundle";
			byte[] bytes2 = Encoding.get_ASCII().GetBytes(text);
			sHA.TransformBlock(bytes2, 0, bytes2.Length, null, 0);
			sHA.TransformFinalBlock(new byte[1], 0, 0);
			byte[] hash = sHA.get_Hash();
			byte[] array = new byte[256];
			Array.Copy(signedBundleData, num + SslSocket.s_magicBundleSignaturePreamble.get_Length(), array, 0, 256);
			...
			{
				RSAParameters key = list.get_Item(j);
		...
		private static bool OnValidateServerCertificate(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
		{
			...
			if (certValidationResult == SslSocket.CertValidationResult.FAILED_CERT_BUNDLE)
			{
				...
				using (List<SslCertBundle>.Enumerator enumerator = list.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						SslCertBundle current = enumerator.get_Current();
						sslSocket.m_bundleSettings.bundle = current;
						...
						{
							FileUtil.StoreToDrive(current.CertBundleBytes, SslSocket.GetBundleStoragePath(), true, true);
							break;
						}
					}
				}
		...
		private static SslSocket.CertValidationResult IsServerCertificateValid(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
		{
			...
			List<string> commonNamesFromCertSubject = SslSocket.GetCommonNamesFromCertSubject(certificate.get_Subject());
			...
			using (List<string>.Enumerator enumerator = commonNamesFromCertSubject.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					string current = enumerator.get_Current();
					if (SslSocket.IsWhitelistedInCertBundle(bundleInfo, current, publicKey))
					{
						flag = true;
						break;
					}
			...
			bool flag4 = !flag2 && flag3 && chain.get_ChainElements().get_Count() == 1;
			try
			{
				if (sslPolicyErrors != null)
				{
					SslPolicyErrors sslPolicyErrors2 = (!flag2 && !flag4) ? 0 : 5;
					if ((sslPolicyErrors & 2) != null && sslSocket.m_connection.MatchSslCertName(commonNamesFromCertSubject))
					{
						sslPolicyErrors2 |= 2;
					}
					if ((sslPolicyErrors & ~(sslPolicyErrors2 != null)) != null)
					{
						...
						SslSocket.CertValidationResult result = SslSocket.CertValidationResult.FAILED_SERVER_RESPONSE;
						return result;
					}
				}
				if (chain.get_ChainElements() == null)
				{
					SslSocket.s_log.LogWarning("ChainElements is null");
					SslSocket.CertValidationResult result = SslSocket.CertValidationResult.FAILED_SERVER_RESPONSE;
					return result;
				}
				X509ChainElementEnumerator enumerator2 = chain.get_ChainElements().GetEnumerator();
				while (enumerator2.MoveNext())
				{
					X509ChainElement current2 = enumerator2.get_Current();
					SslSocket.s_log.LogDebug("Certificate Thumbprint: {0}", new object[]
					{
						current2.get_Certificate().get_Thumbprint()
					});
					X509ChainStatus[] chainElementStatus = current2.get_ChainElementStatus();
				...
				if (flag2)
				{
					if (chain.get_ChainElements().get_Count() == 1)
					{
						chain.get_ChainPolicy().get_ExtraStore().Add(SslSocket.s_rootCertificate);
				...
				{
					using (List<X509Certificate2>.Enumerator enumerator3 = bundleInfo.bundleCerts.GetEnumerator())
					{
						while (enumerator3.MoveNext())
						{
							X509Certificate2 current3 = enumerator3.get_Current();
							chain.get_ChainPolicy().get_ExtraStore().Add(current3);
						}
					}
					chain.Build(new X509Certificate2(certificate));
					flag5 = true;
				}
				for (int j = 0; j < chain.get_ChainElements().get_Count(); j++)
				{
					if (chain.get_ChainElements().get_Item(j) == null)
					{
						SslSocket.s_log.LogWarning("ChainElements[" + j + "] is null");
						SslSocket.CertValidationResult result = (!flag5) ? SslSocket.CertValidationResult.FAILED_SERVER_RESPONSE : SslSocket.CertValidationResult.FAILED_CERT_BUNDLE;
						return result;
					}
				}
				if (flag2)
				{
					...
					if (chain.get_ChainElements().get_Item(1).get_Certificate().get_Thumbprint() != text)
					{
						...
						{
							chain.get_ChainElements().get_Item(1).get_Certificate().get_Thumbprint()
						});
						SslSocket.CertValidationResult result = (!flag5) ? SslSocket.CertValidationResult.FAILED_SERVER_RESPONSE : SslSocket.CertValidationResult.FAILED_CERT_BUNDLE;
						return result;
					}
				}
				for (int k = 0; k < chain.get_ChainElements().get_Count(); k++)
				{
					if (DateTime.get_Now() > chain.get_ChainElements().get_Item(k).get_Certificate().get_NotAfter())
					{
				...
				X509ChainElementEnumerator enumerator4 = chain.get_ChainElements().GetEnumerator();
				while (enumerator4.MoveNext())
				{
					X509ChainElement current4 = enumerator4.get_Current();
					X509ChainStatus[] chainElementStatus2 = current4.get_ChainElementStatus();
					for (int l = 0; l < chainElementStatus2.Length; l++)
					{
						X509ChainStatus x509ChainStatus2 = chainElementStatus2[l];
						if ((!flag2 && !flag5) || x509ChainStatus2.get_Status() != 32)
						{
							SslSocket.s_log.LogWarning("Found unexpected chain error={0}.", new object[]
							{
								x509ChainStatus2.get_Status()
							});
							SslSocket.CertValidationResult result = (!flag5) ? SslSocket.CertValidationResult.FAILED_SERVER_RESPONSE : SslSocket.CertValidationResult.FAILED_CERT_BUNDLE;
							return result;
						}
				...
				X509Certificate2 x509Certificate = new X509Certificate2(chain.get_ChainElements().get_Item(0).get_Certificate());
				...
				{
					x509Certificate.get_FriendlyName()
				});
				SslSocket.s_log.LogDebug("  Public Key Format: {0}", new object[]
				{
					x509Certificate.get_PublicKey().get_EncodedKeyValue().Format(true)
				});
				SslSocket.s_log.LogDebug("  Raw Data Length: {0}", new object[]
				{
					x509Certificate.get_RawData().Length
				});
				SslSocket.s_log.LogDebug("  CN: {0}", new object[]
				{
					x509Certificate.GetNameInfo(3, false)
				});
			}
			catch (Exception ex)
			{
				...
				SslSocket.CertValidationResult result = SslSocket.CertValidationResult.FAILED_CERT_BUNDLE;
				return result;
			}
		...
		private static List<SslCertBundle> DownloadCertBundles(UrlDownloaderConfig dlConfig)
		{
			...
			int numDownloadsRemaining = list.get_Count();
			using (List<string>.Enumerator enumerator = list.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					string current = enumerator.get_Current();
					urlDownloader.Download(current, delegate(bool success, byte[] bytes)
					{
						if (success)
						{
							SslCertBundle sslCertBundle = new SslCertBundle(bytes);
							dlBundles.Add(sslCertBundle);
						}
		...
		private void OnAuthenticateAsClient(IAsyncResult ar)
		{
			...
			try
			{
				...
				{
					this.m_sslStream.get_IsEncrypted(),
					this.m_sslStream.get_IsSigned()
				});
			}
			catch (Exception ex)
			{
		...
		private void ExecuteBeginConnectDelegate(bool connectFailed)
		{
			...
			if (this.m_sslStream != null)
			{
				flag = this.m_sslStream.get_IsEncrypted();
				flag2 = this.m_sslStream.get_IsSigned();
			}
