...
namespace Newtonsoft.Json.Serialization
{
	internal class JsonSerializerInternalWriter : JsonSerializerInternalBase
	{
		...
		private void SerializeValue(JsonWriter writer, object value, JsonContract valueContract, JsonProperty member, JsonContract collectionValueContract)
		{
			...
			{
				((JToken)value).WriteTo(writer, (base.Serializer.Converters == null) ? null : Enumerable.ToArray<JsonConverter>(base.Serializer.Converters));
			}
		...
		private bool ShouldWriteReference(object value, JsonProperty property, JsonContract contract)
		{
			...
			return flag.get_Value() && base.Serializer.ReferenceResolver.IsReferenced(this, value);
		}
		...
		private void WriteMemberInfoProperty(JsonWriter writer, object memberValue, JsonProperty property, JsonContract contract)
		{
			...
			{
				throw new JsonSerializationException("Cannot write a null value for property '{0}'. Property requires a value.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
				{
		...
		private bool CheckForCircularReference(object value, ReferenceLoopHandling? referenceLoopHandling, JsonContract contract)
		{
			...
			case ReferenceLoopHandling.Error:
				throw new JsonSerializationException("Self referencing loop detected for type '{0}'.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
				{
					value.GetType()
				}));
			...
			case ReferenceLoopHandling.Serialize:
				return true;
			default:
				throw new InvalidOperationException("Unexpected ReferenceLoopHandling value: '{0}'".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
				{
		...
		internal static bool TryConvertToString(object value, Type type, out string s)
		{
			...
			if (value is Type)
			{
				s = ((Type)value).get_AssemblyQualifiedName();
				return true;
			}
			s = null;
			return false;
			...
			using (IEnumerator<JsonProperty> enumerator = contract.Properties.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					try
					{
						...
						{
							object value2 = current.ValueProvider.GetValue(value);
							JsonContract contractSafe = this.GetContractSafe(value2);
							this.WriteMemberInfoProperty(writer, value2, current, contractSafe);
						}
					}
					catch (Exception ex)
					{
						if (!base.IsErrorHandled(value, contract, current.PropertyName, ex))
						{
							throw;
						}
		...
		private void SerializeList(JsonWriter writer, IWrappedCollection values, JsonArrayContract contract, JsonProperty member, JsonContract collectionValueContract)
		{
			...
			bool? isReference = contract.IsReference;
			bool flag = (!isReference.get_HasValue()) ? this.HasFlag(base.Serializer.PreserveReferencesHandling, PreserveReferencesHandling.Arrays) : isReference.get_Value();
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					try
					{
						...
						else if (this.CheckForCircularReference(current, default(ReferenceLoopHandling?), contract))
						{
							this.SerializeValue(writer, current, contractSafe, null, collectionValueContract2);
						}
					}
					catch (Exception ex)
					{
		...
		private void SerializeMultidimensionalArray(JsonWriter writer, Array values, JsonArrayContract contract, JsonProperty member, JsonContract collectionContract)
		{
			...
			this._serializeStack.RemoveAt(this._serializeStack.get_Count() - 1);
		...
		private void SerializeMultidimensionalArray(JsonWriter writer, Array values, JsonArrayContract contract, JsonProperty member, int initialDepth, int[] indices)
		{
			...
			{
				array[num] = j;
				bool flag = array.Length == values.get_Rank();
				if (flag)
				{
					...
					try
					{
						...
						else if (this.CheckForCircularReference(value, default(ReferenceLoopHandling?), contractSafe))
						{
		...
		private string GetReference(JsonWriter writer, object value)
		{
			...
			{
				throw new JsonSerializationException("Error writing object reference for '{0}'.".FormatWith(CultureInfo.get_InvariantCulture(), new object[]
				{
					value.GetType()
				}), innerException);
			}
		...
		[SuppressMessage("Microsoft.Portability", "CA1903:UseOnlyApiFromTargetedFramework", MessageId = "System.Security.SecuritySafeCriticalAttribute"), SecuritySafeCritical]
		private void SerializeISerializable(JsonWriter writer, ISerializable value, JsonISerializableContract contract)
		{
			...
			while (enumerator.MoveNext())
			{
				SerializationEntry current = enumerator.get_Current();
				writer.WritePropertyName(current.get_Name());
				this.SerializeValue(writer, current.get_Value(), this.GetContractSafe(current.get_Value()), null, null);
			}
			writer.WriteEndObject();
			this.SerializeStack.RemoveAt(this.SerializeStack.get_Count() - 1);
			contract.InvokeOnSerialized(value, base.Serializer.Context);
		...
		private bool ShouldWriteType(TypeNameHandling typeNameHandlingFlag, JsonContract contract, JsonProperty member, JsonContract collectionValueContract)
		{
			...
			if (this.HasFlag((!typeNameHandling.get_HasValue()) ? base.Serializer.TypeNameHandling : typeNameHandling.get_Value(), typeNameHandlingFlag))
			{
				return true;
			}
			if (member != null)
			{
				...
				if (((!typeNameHandling2.get_HasValue()) ? base.Serializer.TypeNameHandling : typeNameHandling2.get_Value()) == TypeNameHandling.Auto && contract.UnderlyingType != member.PropertyType)
				{
