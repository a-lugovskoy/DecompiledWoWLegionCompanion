...
namespace bnet.protocol.channel
{
	public class UpdateMemberStateNotification : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (this.StateChange.get_Count() != updateMemberStateNotification.StateChange.get_Count())
			{
				return false;
			}
			for (int i = 0; i < this.StateChange.get_Count(); i++)
			{
				if (!this.StateChange.get_Item(i).Equals(updateMemberStateNotification.StateChange.get_Item(i)))
				{
					return false;
				}
			}
			if (this.RemovedRole.get_Count() != updateMemberStateNotification.RemovedRole.get_Count())
			{
				return false;
			}
			for (int j = 0; j < this.RemovedRole.get_Count(); j++)
			{
				if (!this.RemovedRole.get_Item(j).Equals(updateMemberStateNotification.RemovedRole.get_Item(j)))
				{
					return false;
				}
			}
